import { useEffect, useState } from 'react'
import dynamic from 'next/dynamic'
import Countdown from 'react-countdown'
import { Twitter, Discord } from 'components/icons/Link'
import { ScrollToTop, QA, Footer, Navbar } from 'components'
import faqList from 'src/constants/faq'
import tutorialsList from 'src/constants/tutorial'
import Image from 'next/image'
import { disableScroll } from 'src/utils/disableScroll'
import Typewriter from 'typewriter-effect'
import { useIntl } from 'react-intl'
import { getCurrentLanguage } from 'src/utils/language'
import { useRouter } from 'next/router'

const ModalPremint = dynamic(import('components/ModalPremint'))
const ModalStory = dynamic(import('components/ModalStory'))
const ModalProperty = dynamic(import('components/ModalProperty'))
const Modalappplywhite = dynamic(import('components/Modalappplywhite'))

export const endtimestamp = 1664582400000

function App() {
  const intl = useIntl()
  const router = useRouter()
  const currentLang = getCurrentLanguage(router)

  const [headerBackground, setHeaderBackground] = useState(false)
  const [origin, setorigin] = useState()
  useEffect(() => {
    if (typeof window !== 'undefined') {
      setorigin(window.location.origin)
      window.addEventListener('scroll', () =>
        setHeaderBackground(window.pageYOffset > 20)
      )
    }
  }, [])

  return (
    <div className=" min-w-full max-w-full relative overscroll-none overflow-hidden h-full scrollbar-hide">
      <ScrollToTop />
      <Navbar headerBackground={headerBackground} />
      <ModalStory />
      <ModalProperty />
      <ModalPremint />
      <Modalappplywhite />
      <div className="w-full flex flex-col justify-center items-center h-0 ">
        <label
          htmlFor="my-modal-1"
          onClick={() => disableScroll()}
          className="btn modal-button w-[183px] md:w-min btnShadow bg-white
            opacity-80 hover:bg-white hover:opacity-100 px-6  text-sm text-info rounded absolute
            top-[188px] md:top-[408px] md:left-[245px] z-[21] normal-case border-none"
        >
          {intl.formatMessage({ id: 'homepage_map_storyline' })}
        </label>
        <a
          href={`#mindmap`}
          className="btn w-[183px] md:w-max btnShadow bg-white
          opacity-80 hover:bg-white hover:opacity-100 px-6 py-3 mt-4 md:mt-0 text-sm text-info rounded
          absolute top-[236px] md:top-[232px] md:right-1/2 normal-case border-none z-20 "
        >
          {intl.formatMessage({ id: 'homepage_map_mindmap' })}
        </a>
        <a
          href={`#faq`}
          className="btn w-[183px] md:w-max btnShadow bg-white
          opacity-80 hover:bg-white hover:opacity-100 px-6 py-3 mt-4 md:mt-0 text-sm text-info
          rounded absolute top-[300px] md:top-[280px] md:right-1/4 normal-case border-none z-20 "
        >
          {intl.formatMessage({ id: 'homepage_map_faqandtutorials' })}
        </a>
        <label
          htmlFor="property-modal"
          onClick={() => disableScroll()}
          className=" md:hidden btn modal-button w-[183px] md:w-max btnShadow bg-white
          opacity-80 hover:bg-white hover:opacity-100 px-6 py-3 mt-4 md:mt-0 text-sm text-info
          rounded absolute top-[364px] md:top-[280px] md:right-1/4 normal-case border-none z-20 "
        >
          Property Infos
        </label>
        <label
          htmlFor="premint-modal"
          onClick={() => disableScroll()}
          className="btn modal-button w-[183px] md:w-max btnShadow bg-invar-success
          opacity-80 hover:bg-invar-success hover:opacity-100 px-6 py-3 mt-4 md:mt-0 text-sm text-info
          rounded absolute top-[428px] md:top-[449px] md:left-[716px] normal-case border-none z-20 "
        >
          {intl.formatMessage({ id: 'homepage_map_presalemint' })}
        </label>
        <label
          htmlFor="applywhite-modal"
          className=" z-20 absolute top-[512px] md:top-[375px] md:left-[738px] w-[183px] h-[48px] md:w-max btnShadow btn bg-invar-success opacity-80 hover:bg-invar-success hover:opacity-100
            rounded normal-case border-none text-base font-semibold px-[21px] flex flex-col text-[#31135E]"
        >
          <div className=" text-xs ">
            {intl.formatMessage({ id: 'homepage_map_whitelistapply' })}
          </div>
          <Countdown date={endtimestamp} daysInHours={true} />
        </label>
      </div>
      <div className=" w-full min-w-full max-w-full relative bg-gradient-radial from-[#55465D] to-black ">
        {/* <img className=' z-0 h-screen min-h-screen w-full object-cover overflow-hidden' draggable="false" src='/bg/bg.png' alt="bg" /> */}
        <img
          className=" cloud1 absolute top-56 md:top-[161px] -left-16 md:left-0 right-0 w-[500px] md:w-[600px] object-contain z-10 "
          draggable="false"
          src="/cloud1.png"
          alt="cloud1"
        />
        <img
          className=" cloud2 absolute top-[430px] -right-20 md:right-0 w-[600px] md:w-[600px] object-contain z-10 "
          draggable="false"
          src="/cloud2.png"
          alt="cloud2"
        />
        <div className=" relative z-0 h-screen min-h-screen w-full object-cover overflow-hidden">
          <Image
            layout="fill"
            objectFit="cover"
            draggable="false"
            src="/bg/bg.png"
          />
        </div>
        <img
          className=" w-[23%] hidden absolute bottom-0 left-14 z-20 md:block overflow-hidden animate-fade-in-left"
          draggable="false"
          src="/bg/bg_01.png"
          alt="bg_1"
        />
        <label
          htmlFor="property-modal"
          onClick={() => disableScroll()}
          className=" hidden z-30 pr-8 w-48 h-32 hover:cursor-pointer absolute top-[57%] right-[53%] md:flex justify-end items-start"
        >
          <div className=" hidden md:flex justify-center items-center">
            <span className="animate-ping absolute inline-flex h-[14px] w-[14px] rounded-full bg-invar-error opacity-75"></span>
            <span className="relative inline-flex rounded-full h-[10px] w-[10px] bg-invar-error"></span>
          </div>
        </label>
        <div className=" hidden absolute bottom-0 left-0 right-0 z-10 md:flex justify-center items-center">
          <div
            className=" flex justify-start items-start text-start w-[826px] h-[108px] m-6 p-6 bg-invar-main-purple
            bg-opacity-60 text-white text-sm font-normal leading-[19.6px] rounded-lg border-4 border-invar-light-purple
            border-opacity-60 animate-fade-in-up"
          >
            <div className="text-start flex justify-start">
              <Typewriter
                options={{
                  delay: 10,
                }}
                onInit={(typewriter) => {
                  typewriter
                    .pauseFor(1000)
                    .typeString(
                      intl.formatMessage({ id: 'homepage_dialog_man3' })
                    )
                    .start()
                }}
              />
            </div>
          </div>
        </div>
        <div className="m-6 flex justify-between absolute bottom-[0px] right-0 z-20">
          <Twitter />
          <Discord />
        </div>
      </div>
      <div className="relative w-screen min-w-full bg-gradient-to-b from-invar-main-purple to-black text-white pb-12 md:pb-32">
        {/* <img className="absolute top-[px] left-0" src="/bg/mindmap_02.png" draggable="false" alt=""/>
        <img className="absolute top-[px] left-0" src="/bg/mindmap_03.png" draggable="false" alt=""/> */}
        <p className="pt-10 md:pt-16 pb-6 text-2xl md:text-3xl font-semibold text-center mx-16 md:mx-0">
          {intl.formatMessage({ id: 'homepage_intro_title' })}
        </p>
        <div className="mx-[30px] sm:mx-[30px] md:mx-[130px] lg:mx-[230px] font-normal">
          <p>
            <p>{intl.formatMessage({ id: 'homepage_intro_desc1' })}</p>
          </p>
          <br /> <span id="mindmap"></span>
          <p>
            <p>{intl.formatMessage({ id: 'homepage_intro_desc2' })}</p>
          </p>
        </div>
        <p className=" pt-11 md:pt-16 pb-6 text-2xl md:text-3xl font-semibold text-center mx-16 md:mx-0">
          {intl.formatMessage({ id: 'homepage_map_mindmap' })}
        </p>
        <div className=" mx-[30px] sm:mx-[30px] md:mx-[130px] lg:mx-[230px] font-normal">
          <p>
            <p>{intl.formatMessage({ id: 'homepage_mindmap_desc' })}</p>
          </p>
        </div>
        <img
          className=" h-[300px] md:h-[500px] mt-[44px] md:mt-0 md:absolute top-[488px] left-0 z-0 object-cover object-left-top "
          src="/bg/mindmap_02.png"
          draggable="false"
          alt=""
        />
        <div className=" relative md:h-[625px] w-full md:pt-[115px] pb-[53px] md:pb-[228px] flex ">
          <div className=" hidden md:block w-1/2 h-full"></div>
          <div className=" ml-8 text-base font-normal text-white mr-[55px] md:mr-0">
            <p className=" md:font-semibold md:text-invar-light-grey ">
              {intl.formatMessage({ id: 'homepage_mindmap_phaseone' })}
            </p>
            <p className=" text-2xl md:text-[32px] md:leading-[38.4px] font-semibold text-white mt-[4px] md:mt-[10px] mb-6 md:mb-0">
              {intl.formatMessage({
                id: 'homepage_mindmap_phaseone_title',
              })}
            </p>
            {/* <p>
              <br className="hidden md:block" />
              Crypto is all about experiment and exploration. InVar aim to
              <br className="hidden md:block" />
              build first feasible project through tokenization of RWAs.
            </p> */}
            <p>
              {intl.formatMessage({ id: 'homepage_mindmap_phaseone_desc' })}
            </p>
            <br />
            <ul className="list-disc pl-6">
              <li>
                {intl.formatMessage({
                  id: 'homepage_mindmap_phaseone_point1',
                })}
              </li>
              <li>
                {intl.formatMessage({
                  id: 'homepage_mindmap_phaseone_point2',
                })}
              </li>
              <li>
                {intl.formatMessage({
                  id: 'homepage_mindmap_phaseone_point3',
                })}
              </li>
              <li>
                {intl.formatMessage({
                  id: 'homepage_mindmap_phaseone_point4',
                })}
              </li>
            </ul>
          </div>
        </div>
        <img
          className=" h-[300px] md:h-[500px] md:absolute top-[988px] right-[-256.48px] z-0 object-cover object-left-top "
          src="/bg/mindmap_01.png"
          draggable="false"
          alt=""
        />
        <div className=" relative w-full md:pb-[326px] md:mr-[47px] flex justify-end ">
          <div className=" md:h-[272px] md:w-[478px] ml-8 text-base font-normal text-white mr-[55px] md:mr-0">
            <p className=" md:font-semibold md:text-invar-light-grey ">
              {intl.formatMessage({ id: 'homepage_mindmap_phasetwo' })}
            </p>
            <p className=" text-2xl md:text-[32px] md:leading-[38.4px] font-semibold text-white mt-[4px] md:mt-[10px] mb-6 md:mb-0">
              {intl.formatMessage({ id: 'homepage_mindmap_phasetwo_title' })}
            </p>
            {/* <p>
              <br className="hidden md:block" />
              Dive deep to think more about feedback and benefits that can{' '}
              <br className="hidden md:block" />
              bring to early adopters, replicate the successful model and{' '}
              <br className="hidden md:block" />
              develop exciting products.
            </p> */}
            <p>
              {intl.formatMessage({ id: 'homepage_mindmap_phasetwo_desc' })}
            </p>
            <br />
            <ul className=" list-disc pl-6">
              <li>
                {intl.formatMessage({
                  id: 'homepage_mindmap_phasetwo_point1',
                })}
              </li>
              <li>
                {intl.formatMessage({
                  id: 'homepage_mindmap_phasetwo_point2',
                })}
              </li>
              <li>
                {intl.formatMessage({
                  id: 'homepage_mindmap_phasetwo_point3',
                })}
              </li>
            </ul>
          </div>
          <div className=" hidden md:block w-1/2 h-full"></div>
        </div>
        <img
          className="mt-[44px] md:mt-0 h-[300px] md:h-[599px] md:absolute top-[1488px] left-[-129px] z-0 object-cover  "
          src="/bg/mindmap_03.png"
          draggable="false"
          alt=""
        />
        <div className=" relative w-full md:pb-[218px] flex z-10 ">
          <div className=" hidden md:block w-1/2 h-full"></div>
          <div className=" ml-8 md:h-[162px] w-[478px] text-base font-normal text-white mr-[55px] md:mr-0">
            <p className=" md:font-semibold md:text-invar-light-grey ">
              {intl.formatMessage({ id: 'homepage_mindmap_phasethree' })}
            </p>
            <p className=" text-2xl md:text-[32px] md:leading-[38.4px] font-semibold text-white mt-[4px] md:mt-[10px] mb-6 md:mb-0">
              {intl.formatMessage({ id: 'homepage_mindmap_phasethree_title' })}
            </p>
            {/* <p>
              <br className="hidden md:block" />
              InVaria 2222 is the foundation also the digital world of InVar.{' '}
              <br className="hidden md:block" />
              Building solid infrastructure and value backstop to construct a{' '}
              <br className="hidden md:block" />
              hybrid finance ecosystem is a reasonable, indispensable move.
            </p> */}
            <p>
              {intl.formatMessage({
                id: 'homepage_mindmap_phasethree_desc',
              })}
            </p>
            <br />
            <ul className="list-disc pl-6">
              <li>
                {intl.formatMessage({
                  id: 'homepage_mindmap_phasethree_point1',
                })}
              </li>
            </ul>
          </div>
          <span className=" absolute bottom-[60px] " id="faq"></span>
        </div>
        <div className="relative text-white ">
          <p className="pt-10 md:pt-16 pb-6 text-3xl font-semibold text-center">
            FAQ
          </p>
          <div className="relative flex w-full">
            <div className="mx-[30px] sm:mx-[30px] md:mx-[130px] lg:mx-[230px]  w-full z-10 font-normal">
              {faqList(intl, currentLang).map((faq, index) => (
                <QA key={index} {...faq} />
              ))}
            </div>
          </div>
          <div className="pt-11 md:pt-16 pb-6">
            <p className="text-3xl font-semibold text-center">Tutorials</p>
          </div>
          {/* z-index需要有flex */}
          <div className="relative flex w-full">
            <div className="mx-[30px] sm:mx-[30px] md:mx-[130px] lg:mx-[230px] w-full z-10">
              {tutorialsList(intl, currentLang).map((tutorial, index) => (
                <QA key={index} {...tutorial} />
              ))}
            </div>
          </div>
        </div>
      </div>
      <Footer />
    </div>
  )
}

export default App
